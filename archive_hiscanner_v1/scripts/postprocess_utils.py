...
def draw_track(df, baf_alpha = .5):
    ...
    ax[1].scatter(df.index.values, df['pBAF'].values,s=1,color='darkgrey',alpha=baf_alpha)
...
def beta_llk(x, mu, sigma):
    epsilon = 1e-3
    # estimate parameters of beta dist.
    mu = np.max([epsilon, mu])
            ...
            for index, (a,b) in enumerate(cn_pool):
                ...
                for _, BIN in selected_bins.iterrows():
                    if not math.isnan(BIN['VAF_ESTIMATE']):
                        ...
                        # VAF LLK (beta distribution, reparameterized)
                        vaf_llk_corrected= norm.logpdf(BIN.VAF_ESTIMATE, loc=b/(a+b+1e-10), scale=np.sqrt(var_vaf))
                        # x = np.max([BIN.VAF_ESTIMATE, epsilon])
                        # vaf_llk_corrected= beta_llk(x, b/(a+b+epsilon), 0.01)
...
def prep_input_table(cell, hetsnp_dir, bin_dir, chroms = range(1, 23)):
    ## read phased BAF
    fname = os.path.join(hetsnp_dir, f'{cell}.hetsnp.txt')
    ...
    grid = {}
    for i in chroms:
        b = pd.read_csv(f'{bin_dir}/{cell}/{i}.bin', sep='\t')
    ...
    for chrom, g in grid.items():
        for i in range(len(g)):
            start, end = g[i]
            selected = vaf[(vaf['#CHROM']==chrom) & (vaf['POS']>=start) & (vaf['POS']<=end)]
            cell_table.append([chrom, start, end, selected.A.sum(), selected.B.sum(), selected.shape[0]])
    cell_table = pd.DataFrame(cell_table)
    cell_table.columns = ['#CHROM','START','END','A','B','N']
    cell_table.loc[:,'TOTAL'] = cell_table['A'].values+ cell_table['B'].values
    cell_table.loc[:,'pBAF'] = cell_table['B'].values / (1e-10+cell_table['A'].values+ cell_table['B'].values)
    cell_table.loc[:,'BAF'] = [min(1-v,v) for v in cell_table.pBAF.values]
    cell_table.loc[:,'MAJOR'] = [max(a,b) for a,b, in cell_table[['A','B']].values]
    cell_table.loc[:,'MINOR'] = [min(a,b) for a,b, in cell_table[['A','B']].values]
    # annotate rdr
    cell_table['RDR'] = rdr
    cell_table['OBS'] = obs
    cell_table['EXP'] = exp
